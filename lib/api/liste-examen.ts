...
/** Jour calendaire UTC (aligné sur les champs Prisma `@db.Date`). */
export function calendarDayUtc(d: Date): Date {
  return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()))
}

export function calculateAgeAt(dateNaissance: Date, at: Date): number {
  let age = at.getUTCFullYear() - dateNaissance.getUTCFullYear()
  const m = at.getUTCMonth() - dateNaissance.getUTCMonth()
  if (m < 0 || (m === 0 && at.getUTCDate() < dateNaissance.getUTCDate())) age--
  return age
}
...
/** Accepte AAAA-MM-JJ, AAAA/MM/JJ ou ISO — utilisé pour les dates de liste. */
export function parseListeDateOnly(value: unknown, label: string): Date {
  const parsed = parseOptionalListeDateOnly(value)
  if (!parsed) throw new ApiError(400, `${label} invalide.`)
  return parsed
}

/** Date optionnelle (ex. تاريخ آخر امتحان) — retourne null si vide ou illisible. */
export function parseOptionalListeDateOnly(value: unknown): Date | null {
  const s = String(value ?? "").trim()
  if (!s) return null

  const slashOrDash = s.match(/^(\d{4})[/-](\d{1,2})[/-](\d{1,2})/)
  if (slashOrDash) {
    const y = Number(slashOrDash[1])
    const m = Number(slashOrDash[2])
...
export function sameCalendarDayUtc(a: Date, b: Date): boolean {
  return calendarDayUtc(a).getTime() === calendarDayUtc(b).getTime()
}
