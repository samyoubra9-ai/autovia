...
function ApprentissageLayoutInner({ children }: { children: ReactNode }) {
  ...
  const breadcrumb = useMemo(() => {
    ...
    if (moduleSlug && isModuleSlug(moduleSlug)) {
      crumbs.push({
        label: m.modules[moduleSlug as ModuleSlug].title,
        href: `/apprendre/${moduleSlug}`,
      })

      if (parts[2] === "quiz") {
        crumbs.push({ label: m.shell.quizLabel })
      } else if (parts[2] && isChapterInModule(moduleSlug, parts[2])) {
        crumbs.push({
          label: getChapterContent(
            m,
            moduleSlug as ModuleSlug,
            parts[2] as Parameters<typeof getChapterContent>[2],
          ).title,
        })
      }
    }
        ...
        <div className="ap-topbar-inner">
          ...
          <div className="ap-topbar-actions">
            <div className="ap-topbar-progress" title={m.shell.sidebarProgress}>
              <span className="ap-topbar-progress-label">
                {globalPercent}%
              </span>
